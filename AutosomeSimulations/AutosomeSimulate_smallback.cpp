...
int main(int argc, char *argv[])
{
    ...
    //double m=0.00015;
    if (argc==10) // User may run the script with "./a.out RUNS sel DOM", where RUNS = number of runs, sel = selective coefficient (in this work always set to 1), and DOM = heterozygote effect (in this work set to 0 or 1%)
       {
         Uvar=atof(argv[1]);
         //length=atof(argv[2]);
         sel=atof(argv[2]);
    ...
    for(int run=0;run<RUNS;run++){
            ...
            double basedownVAR = (1./(ratio * halfthetaVAR));
		      ...
		      if (((pops[i]->allelenum()>0)&&(pops[i]->allelenum()<(2*pops[i]->size)))||(gen<=taujump)) //If the allele is segregating or we are in recent history (last 5920 generations), introduce mutations
                        {
                            ...
                            pops[i]->mutatedown(boost_poi( mutU*ratio*pops[i]->allelenum())) ;
                        }
