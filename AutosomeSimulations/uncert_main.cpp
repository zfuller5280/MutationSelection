...
/*
int last_size[5] = {20000,100000,613285,1000000,5000000};
std::random_device rd;
std::default_random_engine e1(rd());
std::uniform_int_distribution<int> uniform_dist(0, 4);
int last_pop_size = last_size[uniform_dist(e1)];
*/
...
int main(int argc, char *argv[])
{
    ...
    double m=0.00015;
    if (argc==5) // User may run the script with "./a.out RUNS sel DOM", where RUNS = number of runs, sel = selective coefficient (in this work always set to 1), and DOM = heterozygote effect (in this work set to 0 or 1%)
       {RUNS=atoi(argv[1]);
       sel=atof(argv[2]);
       DOM=atof(argv[3]);
       mutU=atof(argv[4]);
...
int popsize(int gen, int i) //Calculates the size of population i (0=African, 1=European) at generation gen according to Schiffles & Durbin's model
{
  ...
  if ((idx==55) && (randflag==0))
    ...
    //std::default_random_engine e1(rd());
    //std::uniform_int_distribution<int> uniform_dist(0, 4);
    //last_pop_size = last_size[uniform_dist(e1)];
