...
//Functions for random poisson and binomial variables
...
int main(int argc, char *argv[]){
  ...
  if (argc==12)
     ...
     for(int run=0;run<RUNS;run++){
       ...
       // Set the male mutation rate to 3 * U
       double malemutU = 3 * mutU;
       ...
       int total_allele_count = (pops[0]->allelenum()) + (male_pops[0]->allelenum()); // Get the total number of derived alleles
       int total_pop_size = (pops[0]->size + male_pops[0]->size); // Add males and females
