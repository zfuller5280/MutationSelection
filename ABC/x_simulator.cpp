...
int main(int argc, char *argv[]){
  ...
  if (argc==12)
     ...
     for(int run=0;run<RUNS;run++){
       ...
       //cout<<gen<<"\t"<<total_allele_count<<"\t"<<total_pop_size<<"\t"<<(total_allele_count*1.)/total_pop_size<<"\t"<<mutU<<"\n";

       double raw_allele_freq = (total_allele_count*1.)/total_pop_size;
       ...
       if (distance_function==2){
         ...
         //cout << rand_num << " " << raw_allele_freq << " " << sample_allele_count << " " << obs_freq << " " << binom_prob << "\n";
       }
