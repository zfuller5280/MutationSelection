       ...
       while (gen>1){
         //cout<<mutU<<"\t"<<malemutU<<"\n";
          ...
          if ((total_allele_count > 0)||(gen<taujump)){
            ...
            int f_gen = int(-log( BRand::Controller.nextOpened())* (1./(femalemutU * mut_ratio * 2 * pops[0]->size)));
            int m_gen = int(-log( BRand::Controller.nextOpened())* (1./(malemutU * mut_ratio * male_pops[0]->size)));
            //cout<<f_gen<<" "<<m_gen<<"\n";
            gen-=min(f_gen, m_gen);
            if (m_gen<= f_gen){
              male_pops[0]->size=popsize(demographic_model,dem_uncert,gen-1,0);
